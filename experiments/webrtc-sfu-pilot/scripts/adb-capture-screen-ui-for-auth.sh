...
#
# 拉回 Mac：默认 PoC EIP 8.166.118.148（ecs_wayphone）；其它主机: AUTH_DEBUG_SCP_HOST=x.x.x.x
set -euo pipefail
...
ADB_BIN=${ADB_BIN:-adb}
SCP_HOST=${AUTH_DEBUG_SCP_HOST:-8.166.118.148}
...
echo "" >&2
echo "拉回本机对照（PoC EIP ${SCP_HOST}，别名见 docs/ssh-ecs-wayphone.config.example）:" >&2
echo "  scp -i ~/.ssh/miyao.pem root@${SCP_HOST}:${PNG} ~/Downloads/" >&2
[[ -n "${XML}" && -f "${XML}" ]] && echo "  scp -i ~/.ssh/miyao.pem root@${SCP_HOST}:${XML} ~/Downloads/" >&2
echo "  # 若本机已配置 Host ecs_wayphone: scp ecs_wayphone:${PNG} ~/Downloads/" >&2
[[ -n "${XML}" && -f "${XML}" ]] && echo "  # scp ecs_wayphone:${XML} ~/Downloads/" >&2
echo "" >&2
echo "对照实验建议:" >&2
