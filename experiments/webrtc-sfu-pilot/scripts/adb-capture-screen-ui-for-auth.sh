...
#
# 拉回 Mac 示例（脚本结尾会打印）:
#   scp -i ~/.ssh/miyao.pem root@EIP:/path/to/*.png ~/Downloads/
set -euo pipefail
...
echo "" >&2
echo "拉回本机对照（把 EIP、密钥、路径换成你的）:" >&2
echo "  scp -i ~/.ssh/miyao.pem root@你的EIP:${PNG} ~/Downloads/" >&2
[[ -n "${XML}" && -f "${XML}" ]] && echo "  scp -i ~/.ssh/miyao.pem root@你的EIP:${XML} ~/Downloads/" >&2
echo "" >&2
echo "对照实验建议:" >&2
