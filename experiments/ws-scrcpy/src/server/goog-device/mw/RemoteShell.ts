...
export class RemoteShell extends Mw {
    ...
    public createTerminal(params: XtermServiceParameters): IPty {
        ...
        // node-pty >=1.x: use IEvent-style listeners; split senders so TS narrows correctly
        if (USE_BINARY) {
            const send = this.bufferUtf8(5);
            term.onData((data: string | Buffer) => send(data as Buffer));
        } else {
            const send = this.buffer(5);
            term.onData((data: string | Buffer) => send(data as string));
        }
