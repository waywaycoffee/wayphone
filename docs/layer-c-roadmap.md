...
3. 在 **ECS 宿主机**、目录 **`/opt/wayphone/experiments/webrtc-sfu-pilot`** 执行（**推荐，默认**）：  
   **`bash scripts/run-c1-ffmpeg-ingest.sh`** —— 从 **`docker compose logs`** 解析当次 **host/port** 并启动 FFmpeg（容器重启后端口会变，不必手抄）。  
...
| 阶段 | 建议 | 说明 |
|------|------|------|
| **前期** | C2 可先落在 **`webrtc-sfu-pilot` 同一进程**（如 `server.cjs` 增加 HTTP API） | 迭代最快：少端口、少容器、与现有信令/房间状态同机共享容易。 |
| **后期（生产重点改进）** | **拆出独立控制面服务**（独立 Node 或其它运行时），专责 **触控/按键回注 + 鉴权 + 限流** | **故障隔离**（回注逻辑异常不拖死 mediasoup SFU）、**安全边界**（ADB 暴露面单独收紧、审计日志分离）、**扩容与发布节奏** 可与媒体面解耦。 |

**结论**：测试阶段 **不必**为 C2 单独起 Node 服务；**上生产前**将「触控 API 从 pilot 进程拆出」列为 **架构债 / 必做项**，与 **`docs/private-cloud-phone-next-steps.md`** 阶段 4 对齐验收即可。
